...
const StaffDashboard = () => {
  ...
  const handleStatusUpdate = async (ticketId, newStatus, notes = '') => {
    try {
      ...
      // Set success message
      const statusText = newStatus === 'Processing' ? 'started processing' : 'completed';
      setSuccess(`Ticket ${currentTicket?.ticketNumber} has been ${statusText} successfully!`);
      ...
      if ((newStatus === 'Processing' && filter === 'Processing') || 
  ...
  const getPriorityBadge = (priority) => {
    switch (priority) {
      case 'Emergency': return { label: 'EMERGENCY', color: 'bg-red-500 text-white animate-pulse' };
      case 'Elderly': return { label: 'ELDERLY', color: 'bg-purple-500 text-white' };
      case 'Disabled': return { label: 'SPECIAL NEEDS', color: 'bg-indigo-500 text-white' };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 font-medium">Pending Tickets</p>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <p className="text-gray-600 ml-3">Loading {filter.toLowerCase()} tickets...</p>
            </div>
          ) : tickets.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-6xl mb-4">
                {filter === 'Pending' ? '⏳' : filter === 'Processing' ? '🔄' : '✅'}
              </div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No {filter} Tickets</h3>
                ...
                <div key={ticket._id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-bold ${getPriorityBadge(ticket.priority).color}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900 text-sm">{ticket.purpose}</p>
                    </div>
                    <div>
                    ...
                    {ticket.queuePosition && (
                      <div>
                        <p className="text-xs text-gray-500 uppercase tracking-wide">Queue Position</p>
                  ...
                  {ticket.notes && (
                    <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                      <p className="text-xs text-gray-500 uppercase tracking-wide">Staff Notes</p>
                      <p className="text-sm text-gray-700 mt-1">{ticket.notes}</p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center space-x-3">
                    {ticket.status === 'Pending' && (
                      <button
                        onClick={() => handleStatusUpdate(ticket._id, 'Processing')}
                    ...
                    {ticket.status === 'Completed' && (
                      <div className="flex items-center space-x-2 text-green-600">
                        ...
                        {ticket.completedAt && (
                          <span className="text-sm text-gray-500">on {formatDate(ticket.completedAt)}</span>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  {ticket.status === 'Completed' && (ticket.processedAt || ticket.completedAt) && (
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      ...
                      <div className="flex items-center space-x-4 text-xs text-gray-600">
                        ...
                        {ticket.processedAt && (
                          <span>Started: {formatDate(ticket.processedAt)}</span>
                        )}
                        {ticket.completedAt && (
        ...
        {tickets.length > 0 && (
          <div className="mt-8 bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between text-sm text-gray-600">
              <span>Showing {tickets.length} {filter.toLowerCase()} tickets</span>
